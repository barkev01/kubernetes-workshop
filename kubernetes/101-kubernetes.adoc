...
On your computer, run the following command using the `kubectl` command-line tool. In particular, you're going to start a new shell inside a container running `registry.access.redhat.com/ubi9/ubi-minimal` just like you did in the earlier Docker workshop.

[source, bash]
----
$ kubectl run my-shell --rm -it --image registry.access.redhat.com/ubi9/ubi-minimal -- /bin/sh
...
[source, bash]
----
$ kubectl run my-other-shell --rm -it --image registry.access.redhat.com/ubi9/ubi-minimal -- /bin/sh
...
[source, bash]
----
$ kubectl run vars --rm -it --env FOOBAR=BAZBOT --env HELLO=WORLD --image registry.access.redhat.com/ubi9/ubi-minimal -- /bin/sh
sh-4.4# echo $HELLO
WORLD
----
