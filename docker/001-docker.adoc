...
[source, bash]
----
$ docker run --interactive --tty registry.access.redhat.com/ubi9/ubi-minimal /bin/sh
sh-4.4# df
sh-4.4# pwd
...
`registry.access.redhat.com/ubi9/ubi-minimal` is the full url.
`registry.access.redhat.com` is the registry.
`ubi9/ubi-minimal` is the name of the image, with `ubi9` being the _namespace_.
...
[source, bash]
----
$ docker run -it registry.access.redhat.com/ubi9/ubi-minimal /bin/sh
...
[source, bash]
----
$ docker run -it registry.access.redhat.com/ubi9/ubi-minimal echo hello world
----
...
[source, bash]
----
$ docker run -i -t -e HELLO=world registry.access.redhat.com/ubi9/ubi-minimal /bin/sh
sh-4.4# echo $HELLO
----
...
[source, bash]
----
$ docker run -i -t -v $PWD:/host registry.access.redhat.com/ubi9/ubi-minimal ls /host
----
...
When you ran `docker run registry.access.redhat.com/ubi9/ubi-minimal` Docker checked if the image was available locally.
...
You can check https://catalog.redhat.com/en/software/containers/ubi9/ubi-minimal/615bd9b4075b022acc111bf5[the image page] to have all the details you need on the content of that image (Security index, packages...).
...
For example, here you download and then run version 9.7 of the `ubi-minimal` image:

[source, bash]
----
$ docker run registry.access.redhat.com/ubi9/ubi-minimal:9.7 cat /etc/redhat-release
----
...
We'll be building on the `registry.access.redhat.com/ubi9/ubi-minimal` image because it's been tailored for containers.
...
RUN microdnf install -y java-21-openjdk-devel unzip  \
...
[source, bash]
----
$ docker run -it --entrypoint=bash quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-arm64
$ docker run -it --entrypoint=bash quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-amd64
----
...
[source, bash]
----
$ docker run -it --entrypoint=bash quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2
----
...
[source, bash]
----
$ docker manifest inspect quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2
----
...
$ docker manifest create quay.io/cescoffier/my-manifest:1.0 \
    quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-arm64 \
    quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-amd64
$ docker manifest annotate quay.io/cescoffier/my-manifest:1.0 quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-amd64 --arch amd64
$ docker manifest annotate quay.io/cescoffier/my-manifest:1.0 quay.io/quarkus/ubi9-quarkus-graalvmce-builder-image:jdk-21.0.2-arm64 --arch arm64
